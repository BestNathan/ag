...
function Encrypt(buf, key) {
    buf = Buffer.from(buf);
    const len = buf.length;
    const ptr = Y7i(len + 12);
    const bytes = new Uint8Array(Buffer.concat([buf, Buffer.alloc(12)]));
    y1h.HEAPU8.set(bytes, ptr);
    y1h._Encrypt.apply(null, [0, ptr, len, len + 12, key, 0, 0]);
    const res = y1h.HEAPU8.subarray(ptr, ptr + len + 12);
