...
class BacSocket extends WebsocketBase {
    ...
    get url() {
        return `ws://${config.websocketUrl}:${config.getPort(this.vid)}`
    }
    get respMap() {
        if (this._respMap) {
        ...
        map.set(RESET_SHOECODE_R, RestShoecodeResp);
        map.set(GAME_BET_R, GameBetResp);
        map.set(GAME_PAYOUT_ME_R, GamePayoutMeResp);
